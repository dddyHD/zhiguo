...
class UcenterMemberModel extends Model
{
    ...
    protected  function rand_email()
    {
        ...
        if ($this->where(array('email' => $email))->select()) {
           return $this->rand_email();
        } else {
            return $email;
        }
    }

    public function changePhone($uid,$value){
       $result=preg_match("/^(1[3|4|5|7|8])[0-9]{9}$/", $value, $result);
        if(!$result){
            return false;
        }
        $res=$this->where(array('mobile'=>$value))->count();
        if($res){
            return false;
        }
        $rs=$this->where(array('id'=>$uid))->setField('mobile',$value);
        if($rs!==false){
