...
class FileController extends Controller
{
    ...
    public function uploadPictureByWangEditor()
    {
        $data = array();
        foreach ($_FILES as $k => $v) {
            $data[$k] = '@' . realpath($v['tmp_name']) . ";type=" . $v['type'] . ";filename=" . $v['name'] . '.jpg';
        }
        $rs = D('Capi/Call')->callApi('/Sapi/uploadimage', $data);
        $rs = json_decode(htmlspecialchars_decode($rs), true);
        exit($rs['data']['file']['path']);

        //  exit($rs);
    }
    public function uploadVoiceByVoice($path){
        $url=$path;
        $data=array(
            'url'=>$url,
        );
        $rs = D('Capi/Call')->callApi('/sapi/core/uploadVoiceByVoice', $data);
        $rs = json_decode(htmlspecialchars_decode($rs), true);
        return $rs;
    }
    ...
    public function uploadPictureBase64()
    {
        ...
        $data = array();
        $data['data']=$aData;
        $rs = D('Capi/Call')->callApi('/Sapi/uploadBase64', $data);
        exit($rs);
