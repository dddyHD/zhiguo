...
class ModuleModel extends Model
{
    ...
    public function getAll($is_installed = '')
    {
        ...
        $module = S('m_module_all'.$is_installed);
        if ($module === false) {
            ...
            S('m_module_all'.$is_installed, $module);
        }
        return $module;
    }
    ...
    public function  cleanModulesCache()
    {
        ...
        S('m_module_all', null);
        S('m_module_all1', null);
        S('m_admin_modules', null);
        S('m_ALL_MESSAGE_SESSION',null);
        S('m_ALL_MESSAGE_TPLS',null);
    }
    ...
    {
        S('m_common_module_' . strtolower($name), null);
    ...
    private function addDefaultRule($default_rule, $module_name)
    {
        foreach ($default_rule as $v) {
            $rule = M('MAuthRule')->where(array('module' => $module_name, 'name' => $v))->find();
            if ($rule) {
                $default[] = $rule;
            }
        }
