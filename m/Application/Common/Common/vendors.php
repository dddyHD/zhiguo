...
function modC($key, $default = '', $module = '')
{
    ...
    $result = S($tag);
    if ($result === false) {
        $config = M('MConfig')->field('value')->where(array('name' => '_' . strtoupper($mod) . '_' . strtoupper($key)))->find();
        if (!$config) {
            $result = $default;
        } else {
            $result = $config['value'];
...
function m_modC($key, $default = '', $module = ''){
    $mod = $module ? $module : MODULE_NAME;
    if (MODULE_NAME == "Install" && $key == "NOW_THEME") {
        return $default;
    }
    $tag = 'm_conf_' . strtoupper($mod) . '_' . strtoupper($key);
    $result = S($tag);
    if (empty($result)) {
...
{
    /*$reslt=D('Capi/Call')->callApi('/Sapi/Core/sendSms',array('mobile'=>$mobile,'content'=>$content));
    $reslt=json_decode($reslt,true);
    if($reslt['error_code']==0){
        return true;
    }else{
        return "发送失败!" .$reslt['info'];
    }*/
