...
function article_html($text, $type = 'html')
{
    // 无标签格式
    $text_tags = '';
    //只保留链接
...
{
    $status = M('MAddons')->where(array('name' => $name))->getField('status');
    return $status;
}
 ...
 * @author :  xjw129xjt（駿濤） xjt@ourstu.com
 */
function execute_task($action_name, $uid = 0)
{
    $uid = $uid ? $uid : is_login();
    if (D('Module')->checkInstalled('Tcenter') && $uid) {
        $list = D('Tcenter/Task')->getByAction($action_name);
        foreach ($list as $v) {
            D('Tcenter/TaskExecute')->execTask($v['name'], $uid);
        }
    }
    return true;
...
{
    $channel = S('channel_bottom');
    if (empty($channel)) {
        $map = array('status' => array('gt', -1), 'pid' => 0);
        $channel = D('MChannel')->where($map)->order('sort asc,id asc')->select();
        foreach ($channel as &$v) {
            $v['child'] = D('MChannel')->where(array('status' => array('gt', -1),'pid'=>$v['id']))->select();
        }
        unset($v);
        S('channel_bottom',$channel,60*60*24);
    }
    return $channel;
}
...
function parse_weibo_content($content)
{
    ...
    $content = parse_short_to_url($content);
    return $content;
}
...
function render_url_to_short($str)
{
    // $regex = '@(?i)\b((?:[a-z][\w-]+:(?:/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))@';
    $regex = "(((http://|https://).*?)(\/nb|\s))";
    $urlModel = D('Url');
    $str = preg_replace_callback($regex, function ($matches) use ($urlModel) {
        $short = $urlModel->addUrl($matches[1]);
        return '[url:' . $short . ']';
    }, $str);
    return $str;
}

function parse_short_to_url($str)
{

    $pattern = '/\[url:(\S*?)\]/';
    $urlModel = D('Url');
    $str = preg_replace_callback($pattern, function ($matches) use ($urlModel) {
        return ' <a href="/link/'.$matches[1].'"><i class="iconfont icon-link"></i></a> ';
    }, $str);
...
function get_attach_path($path){
    return (is_bool(strpos($path, 'http://')) && is_bool(strpos($path, 'https://'))) ?  'http://'.str_replace('//','/', $path) : $path;
}

function get_wx_token()
{
    $info = D('Capi/Call')->callApi('/Manager/Wxapi/getWxToken');
 ...
 *记录消费记录
 */
function expense_alendar($behavior,$amount,$uid){

    D('consumption_log')->add(array(
        'uid'=>$uid ? $uid:is_login(),
        'behavior'=> $behavior,
        'create_time'=>time(),
        'amount'=>$amount,
...
{
    require('./ThinkPHP/Library/Vendor/Emojione/autoload.php');
    $client = new \Emojione\Client(new \Emojione\Ruleset());

    $client->imageType = 'png';
    $client->ascii = true;
    $client->imagePathPNG = __ROOT__ . '/Public/emoji/';
    ...
    foreach ($at_users as $e) {
        ...
        if ($user) {
            ...
            if (modC('HIGH_LIGHT_AT', 1, 'Weibo') && !$disabel_hight) {
                $content = str_replace("[at:$e]", " <a class='user-at hl ' ucard=\"$user[uid]\" href=\"$query_user[space_mob_url]\">@$query_user[nickname] </a> ", $content);
            } else {
                $content = str_replace("[at:$e]", " <a ucard=\"$user[uid]\" href=\"$query_user[space_mob_url]\">@$query_user[nickname] </a> ", $content);
            }
 ...
 * @auth nkx nkx@ourstu.com
 */
function get_file_url_forum($id){
    $res = M('File')->where(array('id' => $id))->find();
    $resUrl = '';
    if($res) {
        $resUrl = $res['driver'] == 'local' ? 'http://'.$_SERVER['HTTP_HOST'].str_replace('/index.php','',$_SERVER['SCRIPT_NAME']). $res['savepath'] . $res['savename'] : $res['savepath'];
 ...
 */
function preview_image($content){
    preg_match_all('/<img src=\"(.*?)\"/', $content, $img_src);
    preg_match_all('/<img src=\".*?\/>/', $content, $img_tag);
    foreach ($img_tag[0] as $k => &$v) {
        $content =  str_replace($v, '<div class="preview-image" data-index="'.$k.'" data-img="'.$img_src[1][$k].'">' . $v . '</div>', $content);
    }
