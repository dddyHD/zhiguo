...
//            $this->setSearchPostUrl('');
    ...
    {
        $map = array(-1 => '删除', 0 => '下架', 1 => '上架');
        return $this->key($name, $title, 'status', $map);
    }
    public function keyColumnStatus($name = 'status', $title = '订阅类型')
    {
        $map = array(0 =>' 按年订阅', 1 => '一次性订阅');
        return $this->key($name, $title, 'status', $map);
    }
    ...
    public function  keyDoColumnGoods($getUrl, $text = '专栏商品')
    {
        return $this->keyDoAction($getUrl, $text);
    }
