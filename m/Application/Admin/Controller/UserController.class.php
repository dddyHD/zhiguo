...
//        $map['status'] = array('egt', 0);
    ...
    {  clean_cache(RUNTIME_PATH . 'Cache/Home/');
        if($_POST['star'] == 1){
            $aUid=I('post.uid',0,'intval');
            $aStarTitle= I('post.starTitle');
            $aStarText=I('post.starText','','text');
            $arr=array('uid'=>$aUid,'starTitle'=>$aStarTitle,'starText'=>$aStarText);
            $star=json_encode($arr);
            $configModel = D('Config');
            $data['name'] ='_' . strtoupper(CONTROLLER_NAME) . '_STARUSER';
            $data['type'] = 0;
            $data['title'] = '';
            $data['group'] = 0;
            $data['extra'] = '';
            ...
            $data['value'] =$star;
            $data['sort'] = 0;
            $y=$configModel->where(array('name'=>$data['name']))->getField('id');
            if($y){
                $res=$configModel->where(array('name'=>$data['name']))->save($data);
            }
            else{
                $res=$configModel->add($data);
            }
            if($res){
                $this->success('设置成功');
            }else{
    ...
    public function submitNickname()
    {
        ...
        //密码验证
        $uid = UCenterMember()->login(UID, $password, 4);
    ...
    public function submitPassword()
    {
        ...
        $res = D('User/UcenterMember')->updateUserFields(UID, $password, $data);
        if($res !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
            $return['info'] = D('User/UcenterMember')->getError();
        }
        if ($return['status']) {
            $this->success(L('_CHANGE_PASSWORD_SUCCESS_'));
        } else {
            $this->error(UCenterMember()->getErrorMessage($return['info']));
        }
    }
