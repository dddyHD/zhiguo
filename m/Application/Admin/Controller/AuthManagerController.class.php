...
class AuthManagerController extends AdminController
{
    ...
    {
        $list = $this->lists('MAuthGroup', array('module' => 'admin'), 'id asc');
    ...
    {
        $auth_group = M('MAuthGroup')->where(array('module' => 'admin', 'type' => AuthGroupModel::TYPE_ADMIN))
            ->find((int)$_GET['id']);
    ...
    public function user($group_id)
    {
        ...
        $auth_group = M('MAuthGroup')->where(array('status' => array('egt', '0'), 'module' => 'admin', 'type' => AuthGroupModel::TYPE_ADMIN))
    ...
    public function accessUser()
    {
        ...
        $this->updateRules();
        $auth_group = M('MAuthGroup')->where(array('status' => array('egt', '0'), 'type' => AuthGroupModel::TYPE_ADMIN))
            ->getfield('id,id,title,rules');
