...
class ScoreModel extends Model
{
    ...
    public function addType($data)
    {
        ...
        if($data['type_key']==4){
            $query = "ALTER TABLE  `{$db_prefix}member` ADD  `score" . $res . "` DECIMAL(10,".$data['type_value'].") NULL DEFAULT  '0' COMMENT  '" . $data['title'] . "'";
            D()->execute($query);
        }else{
            $query = "ALTER TABLE  `{$db_prefix}member` ADD  `score" . $res . "` DOUBLE NULL DEFAULT  '0'  COMMENT  '" . $data['title'] . "'";
    ...
    public function editType($data)
    {
        ...
        if($data['type_key']==4){
            $query = "alter table `{$db_prefix}member` modify column `score" . $data['id'] . "` DECIMAL(10,".$data['type_value'].") NULL DEFAULT  '0' COMMENT  '" . $data['title'] . "';";
            D()->execute($query);
        }else{
            $query = "alter table `{$db_prefix}member` modify column `score" . $data['id'] . "` DOUBLE NULL DEFAULT  '0'  COMMENT '" . $data['title'] . "';";
            D()->execute($query);
        }
        return $res;
    }
    ...
    public function isBindWeixin(){
        $uid=get_uid();
        $Weixin=M('sync_login')->where(array('uid'=>$uid))->find();
        if(empty($Weixin)){
            return false;
        }else{
            return true;
